...
def load_scaler():
    """
    Load the saved StandardScaler used during preprocessing.
    """
...
def unnormalize_array(values: np.ndarray, scaler):
    """
    Convert standardized vital signs back to original  units
    """
...
def unnormalize_dataframe(df: pd.DataFrame, scaler) -> pd.DataFrame:
    """
    Unnormalize vital-sign columns in a dataframe.
    """
...
def load_split(split_name: str) -> pd.DataFrame:
    """
    Load a processed train, validation, or test split.
    """
...
def load_metrics() -> pd.DataFrame:
    """
    Load saved model metrics and select best validation-loss epoch for neural models.
    """
...
def load_gru_model(window_size: int, horizon: int):
    """
    Load a saved GRU checkpoint for the selected window and horizon.
    """
...
def load_pc_gru_model(window_size: int, horizon: int):
    """
    Load a saved PC-GRU checkpoint for the selected window and horizon.
    """
...
def get_patient_stays(df: pd.DataFrame) -> pd.DataFrame:
    """
    Return unique patient ICU stays.
    """
...
def make_stay_label(row) -> str:
    """
    Create a readable label for one ICU stay.
    """
...
def predict_gru(window_values_normalized: np.ndarray, window_size: int, horizon: int):
    """
    Generate one GRU prediction from a normalized input window.
    """
    ...
    x = (
        torch.tensor(window_values_normalized, dtype=torch.float32)
        .unsqueeze(0)
        .to(device)
    )
...
def predict_pc_gru(window_values_normalized: np.ndarray, window_size: int, horizon: int):
    """
    Generate one PC-GRU prediction from a normalized input window.
    """
...
def predict_arima(window_values_normalized: np.ndarray, horizon: int):
    """
    Generate one windowed ARIMA prediction from a normalized input window.
    """
...
def unnormalize_predictions(predictions_normalized, scaler):
    """
    Unnormalize all model predictions back to clinical units.
    """
...
def add_event_line(fig, event_time):
    """
    Add vertical dashed line marking the last observed input time.
    """
...
def plot_single_vital(
    ...
    """
    Plot observed history, true future value, and model predictions for one vital sign.
    """

    start_idx = max(0, event_idx - window_size + 1)
...
def plot_mini_vital(
    ...
    """
    Plot compact panel for one vital sign.
    """
...
def make_prediction_table(predictions_display, true_values_display):
    """
    Create local MAE/RMSE table for the selected prediction point.
    """

    rows = []

    for model_name, pred in predictions_display.items():
        local_mae = float(np.mean(np.abs(pred - true_values_display)))
        local_rmse = float(np.sqrt(np.mean((pred - true_values_display) ** 2)))

        rows.append(
            {
                "model": model_name,
                "selected_window_mae": f"{local_mae:.3f}",
                "selected_window_rmse": f"{local_rmse:.3f}",
            }
        )
...
def main():
    ...
    tab_dashboard, tab_metrics, tab_mapping = st.tabs(
        ["Dashboard", "Model Comparison", "System Pipeline"]
    )
    ...
    # event_idx is the last observed timestep included in the input window.
    min_event_idx = window_size - 1
    ...
    # Match VitalWindowDataset indexing:
    # input = values[i : i + window_size]
    # target = values[i + window_size + horizon - 1]
    # Here event_idx is the final input time.
    start_idx = event_idx - window_size + 1
    true_future_idx = event_idx + horizon

    window_values_normalized = (
        stay_df.iloc[start_idx : event_idx + 1][VITAL_COLUMNS]
        .values
        .astype(np.float32)
    )
    ...
    with tab_dashboard:
        ...
        display_df = stay_df_display.iloc[start_idx : true_future_idx + 1][
            display_cols
        ].copy()
            ...
            if subset.empty:
                ...
                for col in ["mae", "rmse", "pearson", "direction_accuracy"]:
                    if col in display_subset.columns:
                        display_subset[col] = display_subset[col].map(
                            lambda x: f"{x:.3f}"
                        )
    ...
    with tab_mapping:
        ...
        st.markdown(
            ...
            **Pipeline used by this dashboard:**
            ...
            4. Select an observed window ending at the prediction point
